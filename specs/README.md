...
* ミニイベントは複数のDJを含む。
* ミニイベントでは、そのイベント内での各DJのブース使用時間を管理する。
* ミニイベントの参加DJ ID と個人DJの DJ ID は一意であり、同じ実在人物を指す場合は同じ値である。
* タイムラインは 15 分単位で管理する。
* ブース数は可変で、ブースごとの利用可能時間と休憩時間を設定できる。
* 希望時間は「早朝」「午前」「午後」「夜」「深夜」「希望無し」から選択し、各ラベルは任意の時間範囲へ割り当てられる。
* 同一人物の重複は、配置可能だが強いエラーとして表示する。
* 希望時間外配置は、配置可能だが弱いエラーとして表示する。
* 特定DJのブース使用回数・ブース使用時間には、個人DJ出演とミニイベント参加を含める。
...
| データ構造・概念整理 | `domain-model.md` |
| CSV取り込み | `csv-import.md`, 必要に応じて `domain-model.md` |
| タイムライン配置・検証・集計 | `scheduling.md`, 必要に応じて `domain-model.md` |
